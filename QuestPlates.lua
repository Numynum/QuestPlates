...
do
    ...
    function E:QUEST_ACCEPTED(_, questID, ...)
    ...
    function E:QUEST_WATCH_LIST_CHANGED(questID)
        E:QUEST_ACCEPTED(nil, questID)
    end
end

local OurName = UnitName('player')
local QuestLogIndex = {} -- [questName] = questLogIndex, this is to "quickly" look up quests from its name in the tooltip
...
function GetQuestProgress(unitID)
    ...
    for i = 3, #tooltipData.lines do
        ...
        local nextLine = tooltipData.lines[i+1] or nil

        if line.type == Enum.TooltipDataLineType.QuestTitle and line.id then -- Tooltip line is a quest header..?
            local text, playerName, progressText
            if nextLine and nextLine.type == Enum.TooltipDataLineType.QuestObjective then
                text = nextLine.leftText
                playerName, progressText = strmatch(text, '^(.-)(.+)$') -- nil or '' if 1 is missing but 2 is there
            else
                text = GetQuestObjectiveInfo(line.id, 1, false)
                playerName = ""
                progressText = text
            end
                ...
                if isQuestText then
                    ...
                    if x and y then
                        ...
                        if progress and progress <= 100 then
                            questID = ActiveWorldQuests[ text ] -- not a guarantee
                            questType = 3
    ...
    if progressGlob and questType ~= 2 then
        ...
        if questLogIndex or questID then
            if questID then
                ...
                if info then
                    for i = 1, GetNumQuestLeaderBoards(questLogIndex) or 0 do
                        local _, objectiveType, finished = GetQuestObjectiveInfo(info.questID, i, false)
                        if not finished and (objectiveType == 'item' or objectiveType == 'object') then
                            Q.lootIcon:Show()
                        end
                    end
            ...
            if questLogIndex then
                local link, itemTexture = GetQuestLogSpecialItemInfo(questLogIndex)
...
function E:OnPlateShow(_, plate, unitID)
    ...
    for i = 1, C_QuestLog.GetNumQuestLogEntries() do
        ...
        if info and not info.isHeader then
            ...
            for objectiveID = 1, GetNumQuestLeaderBoards(i) or 0 do
                local objectiveText = GetQuestObjectiveInfo(info.questID, objectiveID, false)
...
function E:ADDON_LOADED(loadedAddon)
    if loadedAddon == addonName then
        for plate in pairs(addon:GetAllNameplates()) do
